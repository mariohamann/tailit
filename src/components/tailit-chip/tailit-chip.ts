...
export default class TailitChip extends TailitElement(LitElement) {
  ...
  render() {
    ...
    const filledClass = this.filled
      ? 'bg-var-100 border-var-100 text-black'
      : 'border-var-900 border text-var-900';
...
} ${filledClass} inline-flex items-center border justify-center rounded-full text-sm font-medium leading-none whitespace-nowrap h-7 px-3 select-none"
        role=${this.role}
      >
