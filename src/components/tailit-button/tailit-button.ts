...
export default class TailitButton extends TailitElement(HeadlessButton(LitElement)) {
  ...
  private disabledClass = 'opacity-50 pointer-events-none cursor-not-allowed'; 
  ...
  render() {
    return this.renderButton(html`
      <div class="${this.coloringClass()} ${this.variantClass()} ${this.disabled ? this.disabledClass : null} ${this.hasFocus ? 'ring-4 ring-var-300' : ''} inline-flex items-center font-medium rounded-lg whitespace-nowrap text-sm px-5 py-2.5 motion-safe:transition-all">
        <slot></slot>
      </div>
    `);
  }
